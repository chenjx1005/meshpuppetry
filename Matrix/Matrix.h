...
//-------------------------------------------------------------------
	...
	Matrix(const int& r, const int& c, float * p) throw(): row(r), col(c), index(NULL)
	{
		num = new float[r*c];
...
//----------------------format of a matrix---------------------------
//-------------------------------------------------------------------
	inline void torow(Matrix& m) const throw()
	{
		assert(m.row==row && m.col==col);
		for(int i=0; i<row; ++i)
		{
			for(int j=0; j<col; ++j)
				m.num[i*col+j] = num[j*row+i];
		}
	}
	inline void tocol(Matrix& m) const throw()
	{
		assert(m.row==row && m.col==col);
		for(int i=0; i<row; ++i)
		{
			for(int j=0; j<col; ++j)
				m.num[j*row+i] = num[i*col+j];
		}
	}
...
//----------------------------IO related-----------------------------
//-------------------------------------------------------------------
	void write(const char * name) const throw()
	{
		FILE * fp;
		fopen_s(&fp, name, "w");
		assert(fp != NULL);
...
//-------------------------------------------------------------------
	...
	friend bool solve(const Matrix& m, Matrix& v, char t) throw();
